...
const calculateListHeight = ({ freeSolo, searchTerm, onApply, isNextPageLoading, showDescription }: ListProps) => {
  ...
  return (
    LIST_MAX_HEIGHT - [showFreeSolo, applyButton, isNextPageLoading, showDescription].filter(t => t).length * ITEM_SIZE
  );
};
        ...
        return (
          <StyledList key={group.name + index} style={style}>
            {group.items.length > 0 && group.name !== 'no-group' && (
              <StyledListSubheader>{group.name.trim() ? group.name : 'Uncategorized'}</StyledListSubheader>
            )}
