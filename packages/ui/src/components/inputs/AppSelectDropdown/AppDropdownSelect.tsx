...
export const AppDropdownSelect: FC<AppDropdownSelectProps> = ({
  ...
  return (
    <DropdownSelectLayout fieldError={fieldError} fullWidth={rest.fullWidth} tooltipText={tooltipText} margin={margin}>
      {/* @ts-ignore variant */}
      <StyledTextField
        ...
        {...rest}
      />
      <Popover {...defaultDropdownSelectPopoverProps} open={open} anchorEl={anchorEl} onClose={handlePopoverClose}>
        <DropdownSelectList
          {...{
            loadNextPage,
...
export const AppDropdownSelectField = (props: AppDropdownSelectFieldProps) => {
  ...
  return (
    ...
    <AppDropdownSelect
      ...
      {...props}
      selectedItem={value}
      setSelectedItem={item => setValue(name, item, { shouldDirty: true, shouldTouch: true, shouldValidate: true })}
      fieldError={error}
    />
  );
};
