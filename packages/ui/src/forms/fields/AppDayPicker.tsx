  ...
  const datePicker = (
    <LocalizationProvider dateAdapter={AdapterLuxon} locale={locales.en}>
      <DatePicker
                ...
                inputProps: {
                  ...
                  onChange: e => {
                    ...
                    if (!(e.target as HTMLInputElement).value) {
                      setValueDate(null);
                      setValue(name, null);
                      required ? setError?.(name, { message: label + ' is required' }) : clearErrors?.(name);
                    }
                  },
                  ...
                  onKeyUp: ({ key }) => {
                    if (key !== 'a' && key !== 'Control' && (valueDate?.isValid || valueDate === null))
                      closePicker(false);
                  },
        ...
        {...(!mobile && {
          PaperProps: {
            className: clsx(pickerClassName, desktopYearClassName, type === 'week' && weeksClassName(month)),
          },
        })}
        // week picker changes
        {...(type === 'week' && {
          renderDay: (day: DateTime, selectedDates: (DateTime | null)[], pickersDayProps: PickersDayProps<DateTime>) =>
